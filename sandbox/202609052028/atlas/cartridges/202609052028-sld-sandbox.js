...
const schema = 'ventus.grid-compute-receipt.v1';
 ...
 */
function createComputeObserver({ onEvent = () => {}, now = () => Date.now() } = {}) {
...
function show(record){
 ...
 if(badge)badge.textContent='TEST CODE '+caseId+' | ENGINE '+record.status.toUpperCase()+' | '+(record.summary?record.summary.measured_count+' measured | ':'')+visitId;
}
const observer=createComputeObserver({onEvent:record=>{
 ...
 if(['127.0.0.1','localhost'].includes(location.hostname))fetch(new URL('/__testcode/receipt',window.location.href),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(envelope),keepalive:true}).catch(error=>{console.error('Test Code receipt could not be saved',error);});
}});
...
(() => {
  ...
  let currentIndustrialEntity = null;
  ...
  function drawLinks(map, origin, name, tech, links, direction, statedMw, layerLoaded = true) {
    ...
    // Draw the measured transmission result even beyond the 40 km nearby search.
  ...
  function install(map) {
    ...
    async function selectAtActual(origin, name, tech, fromSubstation, statedMw,
      ...
      const measuredLinks = nearestSubstations(origin[0], origin[1], subs);
    ...
    async function selectAt(origin,name,tech,fromSubstation,statedMw,expectedArrivalEpoch=null) {
      const detector=window.__TESTCODE_GRID_DETECTOR__;
      const entity=(tech === 'naei_emitter' ? currentIndustrialEntity : null) || {kind:fromSubstation?'substation':(currentRepdRef?'repd':detector.expected.kind),id:String(currentRepdRef||detector.expected.id)};
    ...
    map.on('click', async (event) => {
      try {
        ...
        const hit = features.find(feature => {
          const properties = feature.properties || {};
          const tech = String(properties.tech || properties.type || '');
          return isProjectTech(tech) || tech === 'naei_emitter' || feature.layer?.id === SUBS_LAYER_ID;
        });
        if (!hit) {
        ...
        const tech = String(properties.tech || properties.type || '');
        // Rendered GeoJSON features are tile-quantised. Recover the selected
        ...
        currentIndustrialEntity = tech === 'naei_emitter'
          ? {kind:'industrial',id:properties.id != null ? String(properties.id) : String(properties.name || '')+'@'+origin.map(n=>n.toFixed(6)).join(',')}
          : null;
