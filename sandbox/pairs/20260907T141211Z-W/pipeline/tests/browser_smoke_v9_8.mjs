...
const baseUrl = process.env.V9_BASE_URL || "http://127.0.0.1:8765/uk_renewables_pipeline/202609071221/";
...
try {
  ...
  /* THE INTERCONNECTORS TAB. Sixteen rows outside the REPD spine, and ALL TECH
     stays 7,680 before and after. Every MAP href carries interconnector= and
     technology=interconnector against the canonical receiver and never a
     repd_ref; the six links with no converter coordinate held say NO MAP with
     the reason in the row. Largest first, and IFA leads at 2,000 MW. */
  await page.locator('#tech [data-technology="interconnector"]').click();
  await page.waitForFunction(() => document.getElementById("tbody")?.dataset.total === "16");
  assert.equal(await page.locator("#tbody tr").count(), 16);
  assert.equal(await page.locator("#tbody tr[data-technology=interconnector]").count(), 16);
  assert.match(await page.locator("#resultsMeta").innerText(), /16 of 16 interconnector records/);
  assert.equal(await page.locator("#v2").innerText(), "16");
  assert.equal(await page.locator("#v1").innerText(), "17,950");
  assert.equal(await page.locator("#v3").innerText(), "2,000");
  const interconnectorRows = await page.evaluate(() => {
    const rows = [...document.querySelectorAll("#tbody tr")];
    return {
      linked: rows.map((tr) => { const a = tr.querySelector("a.atlaslink"); return a ? a.href : ""; }).filter(Boolean),
      noMap: rows.filter((tr) => tr.querySelector(".action-disabled")).length,
      noMapReasons: rows.filter((tr) => tr.querySelector(".action-disabled")).map((tr) => tr.querySelector(".map-note").textContent),
      firstName: rows[0].querySelector("td.site").childNodes[0].textContent.trim(),
      badges: [...new Set(rows.map((tr) => tr.querySelector(".badge").textContent))],
    };
  });
  assert.equal(interconnectorRows.linked.length, 10, "ten interconnectors carry a converter coordinate");
  assert.equal(interconnectorRows.noMap, 6, "six carry none and say so");
  assert.ok(interconnectorRows.noMapReasons.every((r) => r.includes("no converter coordinates")));
  assert.equal(interconnectorRows.firstName, "IFA");
  assert.deepEqual(interconnectorRows.badges, ["Interconnector"]);
  for (const href of interconnectorRows.linked) {
    const url = new URL(href);
    assert.equal(url.origin + url.pathname, "https://ventusltd.github.io/gridatlas/atlas/", href);
    assert.ok(url.searchParams.get("interconnector"), href);
    assert.equal(url.searchParams.get("technology"), "interconnector", href);
    assert.equal(url.searchParams.has("repd_ref"), false, "an interconnector must never claim a REPD reference: " + href);
    assert.ok(url.searchParams.get("latitude") && url.searchParams.get("longitude"), href);
  }
  await page.locator('#tech [data-technology="all"]').click();
  await page.waitForFunction(() => document.getElementById("tbody")?.dataset.total === "7680");
  assert.equal(await page.locator("#tbody tr").count(), 20);
...
console.log("V9.8 browser smoke: PASS (interconnectors tab, committed regional ledger, frozen UK and mobile table)");
