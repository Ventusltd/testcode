...
import { loadInterconnectorsV9_8 } from "../data/interconnectors-v9-8.js";
import {
  buildAtlasInterconnectorLinkV9_8,
  interconnectorMapNoteV9_8,
  interconnectorMapUnavailableReasonV9_8,
} from "../core/atlas-interconnector-link-v9-8.js";

/* INTERCONNECTORS ARE A TAB, NOT A FIFTH REPD TECHNOLOGY.
   ALL TECH is the DESNZ register: 7,680 records, 356,474.09 MW, pinned by
   contract and by every gate. The sixteen GB interconnectors are not in that
   register and are not added to it. They live in their own pinned data product
   (data/v9.8), sit behind their own tab, and count only when that tab is
   selected - the count line says "of 16 interconnector records" so a reader
   never mistakes them for REPD rows. */
...
const COLOURS = Object.freeze({ solar: "#ffff00", bess: "#ffae00", wind_onshore: "#00ffff", wind_offshore: "#0066ff", interconnector: "#0066ff" });
const UNITS = Object.freeze({ solar: "MWp", bess: "MW", wind_onshore: "MW", wind_offshore: "MW", interconnector: "MW" });
const ALLOWED_TECHNOLOGIES = new Set(["all", "solar", "bess", "wind_onshore", "wind_offshore", "interconnector"]);
...
let all = [];
let interconnectors = [];
let interconnectorFailure = "";
function universeFor() {
  return technology === "interconnector" ? interconnectors : all;
}
let filtered = [];
let page = 0;
let metadata = null;
...
/* PROGRESSIVE RENDER — measured, not guessed.
   The unfiltered view is 7,680 rows at 42 elements each: 323,802 DOM elements
   built from 13.7 MB of markup in a single synchronous innerHTML assignment.
   Measured on a desktop on 2026-09-06, that assignment blocks the main thread
   for 3.6 to 4.4 seconds. An iPhone is several times slower again, which is
   the freeze where nothing responds to a tap.

   The fetch is NOT the problem and was measured before being blamed: the first
   partition lands in about 80 ms and all sixteen in 458-571 ms at any
   concurrency between 4 and 16. Only the render is slow.

   So the first rows are painted at once and the remainder is appended a chunk
   per animation frame. Nothing is hidden and nothing is paginated: `all` and
   `filtered` are untouched, so search, the counts and the CSV export continue
   to see every record — which is the trap a pagination that slices the data
   rather than the view walks into.

   A render in flight is cancelled by the next one through renderToken, or
   changing a filter mid-render would append rows from the previous result on
   top of the new one. */
const FIRST_PAINT_ROWS = 100;
const CHUNK_ROWS = 300;
let renderToken = 0;

/* requestAnimationFrame does not fire in a background tab, so a reader who
   opens this in a second tab and comes back would find the table still holding
   only its first hundred rows. A timeout does fire there, throttled, so the
   table finishes either way: frames while the reader is looking, timeouts while
   they are not. */
...
/* AN INTERCONNECTOR ROW. Same eleven cells as a REPD row, so the table, the
   mobile layout and the reachability gate see one shape; different words in
   them, because the facts are different. There is no REPD reference, no
   planning authority and no news signal - news signals are REPD-bound by
   contract - and the row says so rather than printing a dash where a reader
   would infer a fact. The MAP link carries the link's own identity. */
function interconnectorRowHtml(record) {
  const label = LABELS[record.technology];
  const unit = UNITS[record.technology];
  const location = [record.county, record.region].filter(Boolean).join(" · ");
  const news = new URL("https://www.google.com/search");
  news.searchParams.set("q", `${record.name} interconnector`);
  news.searchParams.set("tbm", "nws");
  const href = buildAtlasInterconnectorLinkV9_8(record);
  const mapAction = href
    ? `<a class="action-link atlaslink" target="_blank" rel="noopener" href="${escapeHtml(href)}">MAP ↗</a><div class="map-note">${escapeHtml(interconnectorMapNoteV9_8(record))}</div>`
    : `<span class="action-disabled">NO MAP</span><div class="map-note">${escapeHtml(interconnectorMapUnavailableReasonV9_8(record))}</div>`;
  const identity = record.bmrs_code ? `BMRS ${record.bmrs_code}` : "NO BMRS CODE YET";
  const geometry = record.geometry_status === "valid"
    ? `straight line converter to converter, ${record.span.straight_line_km} km; the midpoint is a label anchor, not a location`
    : record.geometry_status === "gb_end_only"
      ? `GB converter ${record.gb_end.name}; the far converter is not held yet`
      : "no converter coordinates held";
  const gbEnd = record.gb_end && record.gb_end.name ? record.gb_end.name : "GB end not held";
  const farEnd = record.far_end && record.far_end.name ? record.far_end.name : `${record.country} end not held`;
  const flow = record.flow
    ? `${record.flow.net_direction}; ${Math.round(record.flow.net_mwh).toLocaleString("en-GB")} MWh net over ${record.flow.years}`
    : "flow data not wired for this link";
  const lifecycle = record.lifecycle + (record.target_operational ? ` · target ${record.target_operational}` : "");
  return `<tr id="repd-${escapeHtml(record.repd_ref)}" data-repd-updated="" data-technology="interconnector"><td class="site">${escapeHtml(record.name)}<div class="project-meta">${escapeHtml(identity)} · ${escapeHtml(record.gg_project_id)} · NOT A REPD RECORD</div><div class="mobile-extra">${escapeHtml(location)}</div><details class="project-record"><summary>INTERCONNECTOR RECORD</summary><div class="record-grid"><div><b>CONVERTERS</b><span>${escapeHtml(gbEnd)} ↔ ${escapeHtml(farEnd)}</span></div><div><b>GEOMETRY</b><span>${escapeHtml(geometry)}</span></div><div><b>FLOW</b><span>${escapeHtml(flow)}</span></div><div><b>LIFECYCLE</b><span>${escapeHtml(lifecycle)}</span></div><div><b>SOURCE</b><span>data-interconnectors reference list; converter positions from gridatlas</span></div><div><b>NOTES</b><span>${escapeHtml(record.notes || "-")}</span></div></div></details></td><td class="hide-mobile">${escapeHtml(location)}</td><td class="hide-mobile">${escapeHtml(record.operator || "not in reference data")}</td><td><span class="badge" style="background:${COLOURS[record.technology]}">${escapeHtml(label)}</span></td><td>${escapeHtml(record.status)}</td><td class="mw">${record.capacity_mw.toLocaleString("en-GB", { maximumFractionDigits: 2 })} ${unit}</td><td class="hide-mobile reference-cell repd-ref">${escapeHtml(record.bmrs_code || "no BMRS code")}</td><td class="hide-mobile reference-cell globalgrid-ref">${escapeHtml(record.gg_project_id)}</td><td class="hide-mobile reference-cell repd-updated">not a REPD record</td><td><span class="signal none">NO SIGNAL</span><div class="signal-note">news signals are REPD-bound; interconnectors carry none</div></td><td><div class="project-actions">${mapAction}<a class="action-link newslink" target="_blank" rel="noopener" href="${escapeHtml(news.href)}">NEWS ↗</a><button class="copy-id" type="button" data-copy-id="${escapeHtml(record.gg_project_id)}">COPY ID</button></div></td></tr>`;
}

function projectRowHtml(project) {
  if (project.technology === "interconnector") return interconnectorRowHtml(project);
...
/* TWENTY ROWS, AND THE REST BEHIND PAGES AND SEARCH.
 *
 * The previous build painted the first hundred rows and then streamed the
 * remaining seven and a half thousand into the DOM in chunks. That stopped the
 * page freezing, but it still built 7,680 table rows on a phone for a reader
 * who was going to look at the top of the list.
 *
 * So the VIEW is now a page of twenty, largest capacity first. The DATA is
 * untouched: `all` still holds every qualifying record, `filtered` still holds
 * every record matching the current tabs and search, the CSV still exports the
 * filtered set in full, and the count line still states both numbers. Nothing
 * is dropped from the pipeline - the reader is simply not made to render it.
 *
 * The tabs and the search box do the narrowing. Paging is for what is left. */
const PAGE_ROWS = 20;

function pageCount() {
  return Math.max(1, Math.ceil(filtered.length / PAGE_ROWS));
}

function pagerElement() {
  let el = document.getElementById("projectPager");
  if (!el) {
    el = document.createElement("nav");
    el.id = "projectPager";
    el.className = "pager";
    el.setAttribute("aria-label", "Project pages");
    const table = document.getElementById("tbody");
    const anchor = table ? table.closest("table") : null;
    if (anchor && anchor.parentNode) anchor.parentNode.insertBefore(el, anchor.nextSibling);
    else if (table) table.parentNode.appendChild(el);
  }
  return el;
}

function renderPager() {
  const el = pagerElement();
  const pages = pageCount();
  const total = filtered.length;
  if (!total) {
    el.innerHTML = '<span class="pager-meta">no records match the current filters</span>';
    return;
  }
  const from = page * PAGE_ROWS + 1;
  const to = Math.min(total, (page + 1) * PAGE_ROWS);
  el.innerHTML =
    '<button type="button" class="pager-btn" data-page-step="-1"' + (page === 0 ? ' disabled' : '') + '>&larr; PREV</button>' +
    '<span class="pager-meta">' + from.toLocaleString("en-GB") + '&ndash;' + to.toLocaleString("en-GB") +
      ' of ' + total.toLocaleString("en-GB") + ' &middot; page ' + (page + 1) + ' of ' + pages + '</span>' +
    '<button type="button" class="pager-btn" data-page-step="1"' + (page + 1 >= pages ? ' disabled' : '') + '>NEXT &rarr;</button>';
  el.dataset.page = String(page + 1);
...
function renderTable() {
  ...
  const slice = filtered.slice(from, from + PAGE_ROWS);
  body.innerHTML = slice.map(projectRowHtml).join("");
  body.dataset.rendered = String(slice.length);
  body.dataset.total = String(total);
  body.dataset.page = String(page + 1);
  body.dataset.pages = String(pages);
...
function updateResultSummary() {
  ...
  element.textContent = `${summary.count.toLocaleString("en-GB")} of ${universe.length.toLocaleString("en-GB")} ${technology === "interconnector" ? "interconnector records (outside the REPD spine)" : "records"} · ${formatCapacityV9_2(summary.capacity_mw)} MW · largest ${formatLargestV9_2(summary.largest_mw)} MW`;
  element.classList.toggle("is-filtered", summary.count !== universe.length);
  element.dataset.filteredCount = String(summary.count);
  element.dataset.totalCount = String(universe.length);
}
...
function apply({ syncUrl = true } = {}) {
  ...
  filtered = universeFor().filter((project) => projectMatchesV9_2(project, {
    technology,
    status,
    county,
    tokens,
...
function downloadCsv(event) {
  ...
  anchor.href = url;
  anchor.download = `globalgrid2050_uk_renewables_pipeline_v9_8_${new Date().toISOString().slice(0, 10)}.csv`;
  anchor.click();
...
export async function loadProjectsV9_8() {
  try {
    ...
    all = [...model.projects];
    /* Loaded beside the spine, never merged into it. A failure here disables
       the INTERCONNECTORS tab with its reason and leaves the 7,680 untouched:
       the register must not fail closed because a sixteen-row product did. */
    try {
      const interconnectorModel = await loadInterconnectorsV9_8();
      interconnectors = [...interconnectorModel.records];
    } catch (error) {
      interconnectors = [];
      interconnectorFailure = String((error && error.message) || error);
      console.error(error);
      const tab = document.querySelector('#tech [data-technology="interconnector"]');
      if (tab) { tab.disabled = true; tab.title = interconnectorFailure; }
    }
    state.interconnectors = interconnectors;
    filtered = all;
    metadata = model.metadata;
    release = model.release;
    searchIndex = new Map(all.map((project) => [project.repd_ref, buildProjectSearchTextV9_2(project)]));
    for (const record of interconnectors) searchIndex.set(record.repd_ref, buildProjectSearchTextV9_2(record));
    state.all = all;
    state.filtered = filtered;
    ...
    apply({ syncUrl: false });
    document.getElementById("releaseMeta").textContent = `V9.8 interface · V${release.data_parent.release} canonical data spine · ${interconnectors.length} interconnectors on their own tab · all ${all.length.toLocaleString("en-GB")} qualifying records loaded`;
    ...
    verifying.then((result) => {
      if (result.changed) refreshProjectsV9_8();
      syncMapAtlasNavV9_7();
...
export function refreshProjectsV9_8() {
  if (metadata) renderTable();
}

export function bindProjectControlsV9_8() {
  ...
  /* Delegated, because the pager is rebuilt on every render. Paging moves the
     WINDOW over `filtered`; it never re-filters and never re-fetches. */
  document.addEventListener("click", (event) => {
    const button = event.target.closest ? event.target.closest("[data-page-step]") : null;
    if (!button || !document.getElementById("projectPager")) return;
    const step = Number(button.getAttribute("data-page-step"));
    if (!Number.isFinite(step)) return;
    const pages = pageCount();
    const next = Math.min(pages - 1, Math.max(0, page + step));
    if (next === page) return;
    page = next;
    renderTable();
    const table = document.getElementById("tbody");
    if (table && table.closest("table")) table.closest("table").scrollIntoView({ block: "start" });
  });
