...
const baseUrl = process.env.V9_BASE_URL || "http://127.0.0.1:8765/uk_renewables_pipeline/v9.5/";
...
try {
  ...
  assert.equal(await page.locator("#releaseMeta").textContent(), "V9.5 interface · V9.1 canonical data spine · all 7,680 qualifying records loaded");
  assert.match(await page.locator("#newsMeta").textContent(), /canonical PRIMARY_MATCH headlines/);
  ...
  await page.locator("#search").fill("definitely-no-such-repd-project-v9-5");
  assert.equal(await page.locator("#tbody tr").count(), 0);
  const downloadPromise = page.waitForEvent("download");
  await page.locator("#exportInline").click();
  const download = await downloadPromise;
  assert.match(download.suggestedFilename(), /^globalgrid2050_uk_renewables_pipeline_v9_5_\d{4}-\d{2}-\d{2}\.csv$/);
  ...
  await newsFailure.page.route("**/dist/major_project_news_v6.json*", (route) => route.abort());
  ...
  await newsFailure.page.waitForFunction(() => document.querySelector("#newsMeta").textContent === "canonical feed unavailable");
  ...
  await projectFailure.page.route("**/contracts/release.v9.5.json*", (route) => route.abort());
  ...
  await projectFailure.page.waitForFunction(() => document.querySelector("#tbody").textContent.includes("V9.5 has failed closed."));
  ...
  console.log("V9.5 browser smoke: PASS (canonical REPD news binding, clickable date header, CSV and Atlas)");
} finally {
  await browser.close();
}
