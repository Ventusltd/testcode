...
const baseUrl = process.env.V9_BASE_URL || "http://127.0.0.1:8765/uk_renewables_pipeline/v9.6.1/";
const parentUrl = new URL("../v9.5.1/", baseUrl).href;
...
async function layoutSnapshot(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      headerCellDisplays: [...table.tHead.rows[0].cells].map((cell) => getComputedStyle(cell).display),
      firstRowCellDisplays: [...firstRow.cells].map((cell) => getComputedStyle(cell).display),
      bounds,
    };
  });
}

async function renderedHash(page, selector) {
  return page.locator(selector).evaluate(async (element) => {
    const bytes = new TextEncoder().encode(element.innerHTML);
...
try {
  ...
  assert.equal(await page.locator("#minCapacity").count(), 0);
  assert.equal(await page.locator("#maxCapacity").count(), 0);
  assert.equal(await page.locator("#releaseMeta").textContent(), "V9.5.1 interface · V9.1 canonical data spine · all 7,680 qualifying records loaded");
  ...
  for (const width of [390, 430, 440, 768]) {
    ...
    assert.equal(mobile.tableDisplay, "table", `${width}px native table`);
    assert.equal(mobile.headDisplay, "table-header-group", `${width}px native table head`);
    assert.equal(mobile.rowDisplay, "table-row", `${width}px native project row`);
    assert.equal(mobile.tableMinWidth, "1280px", `${width}px inherited V9.5.1 table minimum`);
    assert.equal(mobile.wrapOverflowX, "auto", `${width}px project wrapper overflow`);
    assert.equal(mobile.headerCellDisplays.length, 11, `${width}px complete project headers`);
    assert.equal(mobile.firstRowCellDisplays.length, 11, `${width}px complete project row`);
    assert.ok(mobile.headerCellDisplays.every((display) => display === "table-cell"), `${width}px visible project headers`);
    assert.ok(mobile.firstRowCellDisplays.every((display) => display === "table-cell"), `${width}px visible project cells`);
    assert.ok(mobile.tableScrollWidth > mobile.wrapClientWidth, `${width}px projects are horizontally scrollable`);
    assertContained(mobile, width, `${width}px mobile`);
    const scrolled = await page.locator(".tablewrap").evaluate((wrap) => {
      wrap.scrollLeft = wrap.scrollWidth;
      return wrap.scrollLeft;
    });
    assert.ok(scrolled > 0, `${width}px project table accepts a horizontal swipe`);
  }

  const parentPage = await context.newPage();
  await parentPage.addInitScript(() => {
    window.Chart = class ChartStub {
      constructor(_canvas, config) { this.data = config.data; }
      update() {}
    };
  });
  await parentPage.route("https://cdn.jsdelivr.net/**", (route) =>
    route.fulfill({ status: 200, contentType: "application/javascript", body: "" }));
  await parentPage.route("https://raw.githubusercontent.com/**", (route) => route.abort());
  await parentPage.goto(parentUrl, { waitUntil: "domcontentloaded" });
  await parentPage.waitForFunction(() => document.querySelectorAll("#tbody tr").length === 7680, null, { timeout: 60000 });
  await parentPage.waitForFunction(() => document.querySelectorAll("#stories .story").length === 133, null, { timeout: 30000 });
  assert.equal(await renderedHash(page, "#tbody"), await renderedHash(parentPage, "#tbody"), "project rendering differs from V9.5.1");
  assert.equal(await renderedHash(page, "#stories"), await renderedHash(parentPage, "#stories"), "newspaper rendering differs from V9.5.1");
  for (const width of [769, 800, 900, 920, 921, 1000, 1100, 1101, 1200, 1440]) {
    await page.setViewportSize({ width, height: 900 });
    await parentPage.setViewportSize({ width, height: 900 });
    const targetLayout = await layoutSnapshot(page);
    const parentLayout = await layoutSnapshot(parentPage);
    for (const key of ["bodyDisplay", "gaugeColumns", "storyColumns", "headerDirection", "statusWhiteSpace", "searchWidth", "clientWidth", "scrollWidth", "tableDisplay", "headDisplay", "rowDisplay", "tableMinWidth", "tableScrollWidth", "wrapClientWidth", "wrapOverflowX"]) {
      assert.deepEqual(targetLayout[key], parentLayout[key], `${width}px desktop parity: ${key}`);
    }
  }
  ...
  await page.locator("#search").fill("definitely-no-such-repd-project-v9-5-1");
  assert.equal(await page.locator("#tbody tr").count(), 0);
  const downloadPromise = page.waitForEvent("download");
  await page.locator("#exportInline").click();
  const download = await downloadPromise;
  assert.match(download.suggestedFilename(), /^globalgrid2050_uk_renewables_pipeline_v9_5_1_\d{4}-\d{2}-\d{2}\.csv$/);
  ...
  await projectFailure.page.route("**/contracts/release.v9.5.1.json*", (route) => route.abort());
  ...
  await projectFailure.page.waitForFunction(() => document.querySelector("#tbody").textContent.includes("V9.5.1 has failed closed."));
  ...
  console.log("V9.6.1 browser smoke: PASS (V9.5.1 parity plus mobile project-table scroll)");
} finally {
  await browser.close();
}
