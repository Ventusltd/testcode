...
/* D-01, "the MAP button does not fire the grid engine", asserted rather than
   reported. Every case is placed in exactly one state from its own sentinels.
   The states beginning OK_ are the legitimate reasons an arrival produces no
   distance, named so that none of them can quietly absorb a real failure:
   a handover to the interconnector lane, a register row with no coordinate,
   a genuine nearest lying beyond the search budget. Anything else with an
   identity resolved and mapped and an engine that said nothing is the disease,
   and it fails the run. */
function diseaseState(c) {
  const id = c.identity, e = c.engine || {}, ic = c.interconnector;
  if (ic) return (e.arrival_reconciliation === "HANDED_TO_INTERCONNECTORS" || ic.status) ? "OK_INTERCONNECTOR_LANE" : "D-01_SUSPECT_INTERCONNECTOR_ARRIVAL_SILENT";
  if (!id) return "NOT_APPLICABLE";
  if (id.status === "RESOLVED_UNMAPPED" || id.mapped === false) return "OK_NO_COORDINATE";
  if (id.status !== "RESOLVED") return "NOT_APPLICABLE";
  if (Number.isFinite(e.nearest_km) || (e.links_drawn || 0) > 0 || c.nearest_on_page === true) return "OK_ENGINE_MEASURED";
  if (e.nearest_beyond_radius === true) return "OK_BEYOND_SEARCH_RADIUS";
  return "D-01_SUSPECT_ENGINE_SILENT_ON_MAPPABLE_PROJECT";
}
...
if (!control.pass) {
  ...
  // classify every case, including the control, and let a suspect fail the run
  receipt.diseases = Object.fromEntries([control, ...receipt.cases].filter(Boolean).map((c) => [c.name, diseaseState(c)]));
  const suspects = Object.entries(receipt.diseases).filter(([, v]) => v.startsWith("D-01"));
  for (const c of [control, ...receipt.cases]) {
    if (receipt.diseases[c.name] && receipt.diseases[c.name].startsWith("D-01") && c.pass === true) c.pass = false;
    if (receipt.diseases[c.name] && receipt.diseases[c.name].startsWith("D-01")) c.disease_failure = receipt.diseases[c.name];
  }
  ...
  receipt.outcome = (suspects.length || !scored.every((c) => c.pass)) ? "FAIL" : gaps.length ? "INCOMPLETE" : "PASS";
  receipt.coverage = { scored: scored.length, passed: scored.filter((c) => c.pass).length, not_implemented: gaps.map((c) => c.name) };
  receipt.statement = `${scored.filter((c) => c.pass).length} of ${scored.length} scored cases passed with a valid control; ${gaps.length} requested behaviour(s) NOT_IMPLEMENTED; ${suspects.length} case(s) classified D-01. The run is ${receipt.outcome}.`;
}
