...
(() => {
  ...
  /* Coordinates the register does not carry, derived from Crown Estate lease
     geometry by tools/offshore/reconcile_offshore_coordinates.py and published
     beside the Atlas. Loaded once, lazily, and never treated as authoritative:
     it locates a project the register has already named.

     Loaded over XMLHttpRequest rather than fetch, deliberately. The composer
     executes cartridges from a blob: origin, where fetch resolves 200 and then
     throws TypeError reading the body; XHR against the same absolute URL
     returns the file. Measured, in this Atlas, twice. */
  let derivedIndex = null;
  function loadDerivedCoordinates() {
    if (derivedIndex) return derivedIndex;
    derivedIndex = new Promise((resolve) => {
      try {
        const url = new URL('../../data/offshore-coordinates.json', document.baseURI).href;
        ...
        xhr.onload = () => {
          ...
          try {
            ...
            const index = {};
            for (const row of payload.coordinates || []) {
              if (row && row.repd_ref && Number.isFinite(row.longitude) && Number.isFinite(row.latitude)) {
                index[String(row.repd_ref)] = row;
              }
            }
            resolve(index);
          } catch (_) { resolve({}); }
        };
        // An absent or unreachable file is not an error here. It means no
        // derived point is available, which is the state this code replaces.
        xhr.onerror = () => resolve({});
        xhr.send();
      } catch (_) { resolve({}); }
    });
  ...
  async function derivedCoordinateFor(repdRef) {
    const index = await loadDerivedCoordinates();
    const row = index[String(repdRef)];
    if (!row) return null;
    const candidate = { longitude: row.longitude, latitude: row.latitude };
    // The same safety test the register's own points must pass.
    if (!hasSafeMapPoint(candidate)) return null;
    return {
      longitude: row.longitude,
      latitude: row.latitude,
      source: row.method || 'DERIVED',
      lease: row.matched_lease || null,
  ...
  async function receiveExactRepdDeepLink(input, resultsEl,
    ...
    try {
      ...
      /* A ref the register does not locate used to be treated worse than a ref
         the register does not contain. The not-in-register branch above keeps
         the supplied arrival point and degrades gracefully; this branch found
         its row, could not map it, and threw - so a project present in the
         register but without a coordinate showed the user a failure card.
         Measured on the live page: between 3 and 8 of 20 MAP links carry no
         coordinate at all, so there was nothing to fall back to either.

         Offshore is where this bites. 8 of 101 offshore rows carry no easting
         or northing, and the Crown Estate leases the seabed under them, so a
         derived point exists for 6 of the 8 in atlas/data/offshore-coordinates.json.
         Using it is honest as long as the card says the coordinate is derived
         and the identity is not: the register named the project, the lease
         located it. */
      const derived = !hasSafeMapPoint(exact) ? await derivedCoordinateFor(repdRef) : null;
      if (derived) {
        exact.longitude = derived.longitude;
        exact.latitude = derived.latitude;
      }
      /* No register point and no lease point. Measured offline on v9.144:
         13432 is in the register, has no coordinate, is in no lease, and
         still reached the invariant below and showed a failure card. A name
         the register confirmed is worth more than a card that says try again,
         so the identity stands and the map does not move. If the link itself
         carried a usable point it is used and labelled as the link's, never
         as the register's. */
      const linkPoint = !hasSafeMapPoint(exact) && hasSafeMapPoint(arrival)
        ? { longitude: arrival.longitude, latitude: arrival.latitude } : null;
      if (linkPoint) {
        exact.longitude = linkPoint.longitude;
        exact.latitude = linkPoint.latitude;
      }
      if (!hasSafeMapPoint(exact)) {
        document.body.dataset.gridatlasRepdRef = repdRef;
        document.body.dataset.gridatlasRepdDeepLink = 'resolved-unmapped';
        state.deep_link = {
          ...arrival,
          owner_epoch: ownerEpoch,
          repd_ref: repdRef,
          status: 'RESOLVED_UNMAPPED',
          resolved: true,
          mapped: false,
          name: exact.name,
          ...
          coordinate_lease: null,
          coordinate_tenant: null,
          coordinate_statement: 'The register names this project but holds no coordinate for it, and no Crown Estate lease covers it. Identity confirmed; position unknown; the map was not moved.',
          statement: 'Exact identity in the active-register snapshot; no coordinate from any source held.'
        };
        return;
      }
      ...
      state.deep_link = {
        ...
        resolved: true,
        mapped: true,
        /* Identity and coordinate have different provenance and are reported
           separately. The register named this project; where it did not locate
           it, a Crown Estate lease did, and the card must not present the two
           as one fact. */
        coordinate_source: derived ? derived.source : linkPoint ? 'ARRIVAL_LINK' : 'OFFICIAL_ACTIVE_REGISTER',
        coordinate_derived: Boolean(derived),
        coordinate_lease: derived ? derived.lease : null,
        coordinate_tenant: derived ? derived.tenant : null,
        coordinate_statement: derived
          ? 'Position derived from Crown Estate lease geometry because the register carries no coordinate for this project. The identity is the register’s; the position is not.'
          : linkPoint
            ? 'Position taken from the arrival link because the register carries no coordinate for this project and no lease covers it. The identity is the register’s; the position is the link’s.'
            : 'Position as published in the active register.',
        name: exact.name,
