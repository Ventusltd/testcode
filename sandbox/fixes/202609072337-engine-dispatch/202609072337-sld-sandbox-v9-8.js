...
(() => {
  ...
  function drawLinks(map, origin, name, tech, links, direction, statedMw, layerLoaded = true) {
    ...
    if (direction !== 'from-substation') {
      ...
      link.arrival_engine = {
        engine: offshore ? 'offshore' : 'onshore',
        answered: links.length > 0 || Boolean(links.beyondRadiusNearest),
        reason: links.length ? null
          : (links.beyondRadiusNearest ? 'NEAREST_BEYOND_' + MAX_LINK_KM + '_KM' : 'NO_SUBSTATION_DATA'),
        fallback_from: offshore && !currentDeclared ? 'declared_connection_point' : null,
  ...
  function install(map) {
    ...
    function answerArrival(engine, answered, reason, fallbackFrom) {
      link.arrival_engine = {
        engine: engine || 'onshore',
        answered: Boolean(answered),
        reason: answered ? null : (reason || 'UNSTATED'),
      ...
      try {
        ...
        if (q.get('interconnector') || String(q.get('technology') || '') === 'interconnector') {
          link.arrival_reconciliation = {
            /* Answered by the interconnector engine, not unanswered. */
        ...
        if (!coordsUsable()) {
          try { answerArrival(classifyEngine(String(tech || ''), q), false, 'NO_USABLE_COORDINATES', null); } catch (_) {}
          return;
        }
  ...
  function arrive(map, geojson) {
    ...
    if (!link || !line) {
      ...
      const usable = Number.isFinite(lon) && Number.isFinite(lat) && Math.abs(lon) <= 180 && Math.abs(lat) <= 90
        && !(Math.abs(lon) < 1e-9 && Math.abs(lat) < 1e-9);
      if (!usable) { state.arrival.framed = false; return; }
