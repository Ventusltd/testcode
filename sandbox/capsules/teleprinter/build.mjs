...
if (mode === 'prepare') {
  ...
  try {
    ...
    for (const file of ['host.js','dismissal.js','focus-boundary.js','readiness.js','viewport.js']) await write('tool-layers/'+file, (await readFile(new URL('../tool-layers/'+file,import.meta.url),'utf8')).replace(/\r\n/g,'\n'));
  ...
  for (const app of ['landing', 'pipeline', 'atlas']) {
    ...
    if (app === 'atlas') { selected.push('atlas/current.json'); if(toolLayerBootstrap) selected.push('atlas/tool-layers.json','tool-layers/host.js','tool-layers/dismissal.js','tool-layers/focus-boundary.js','tool-layers/readiness.js','tool-layers/viewport.js'); }
