...
const modules = ['controls.js', 'print-screen.js', 'screen-pdf.mjs', 'png-pixels.mjs', 'print-source-code.js', 'runtime-source.js', 'app-frame.js', 'capture-geometry.mjs', 'clone-visible-ui.js', 'layer-quick-controls.js', 'layers-panel-policy.js', 'layout-command.js', 'cartridges.json', 'vendor/html2canvas-1.4.1.mjs', 'vendor/html2canvas-LICENSE.txt'];
...
if (mode === 'prepare') {
  ...
  try {
    ...
    for (const file of ['host.js','dismissal.js','focus-boundary.js','readiness.js','viewport.js','session-restart.js']) await write('tool-layers/'+file, (await readFile(new URL('../tool-layers/'+file,import.meta.url),'utf8')).replace(/\r\n/g,'\n'));
  ...
  for (const app of ['landing', 'pipeline', 'atlas']) {
    ...
    if (app === 'atlas') { selected.push('atlas/current.json'); if(toolLayerBootstrap) selected.push('atlas/tool-layers.json','tool-layers/host.js','tool-layers/dismissal.js','tool-layers/focus-boundary.js','tool-layers/readiness.js','tool-layers/viewport.js','tool-layers/session-restart.js'); }
