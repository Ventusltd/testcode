...
export function observeToolReadiness(tool, frame, status, {timeout=30000,interval=200}={}) {
  ...
  const started = () => {
    clearTimeout(timer);delete status.dataset.timedOut;const deadline=Date.now()+timeout;
    const check = () => {
      if(disposed)return;
      let result;
      try {result=inspectToolReadiness(tool.id,frame.contentDocument);} catch {result={interface:'unavailable',drawing:'unreported',label:'Tool opened on another site'};}
      status.textContent=result.label;status.dataset.interface=result.interface;status.dataset.drawing=result.drawing;
      if(result.drawing==='ready' || result.drawing==='unreported')return;
