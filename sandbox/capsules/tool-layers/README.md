...
`source-browser/` provides a file-at-a-time source reader, opened by the layer's Source code button. It selects the currently resolved tool, verifies each response against the served inventory, rejects redirects or byte mismatches, and displays source with textContent so HTML and JavaScript remain text. Build and served-byte gates establish producer provenance; the browser reader's narrower claim is agreement with that served inventory. No concatenated source document is generated.
