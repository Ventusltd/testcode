...
(async()=>{const browser=await pw[engine].launch();try{for(const viewport of [{width:320,height:568},{width:393,height:852},{width:1440,height:900}]){
...
}}finally{await browser.close();}})().catch(e=>{reports.push({error:e.stack});process.exitCode=1;}).finally(()=>{fs.writeFileSync(path.join(out,'results.json'),JSON.stringify({base,engine,reports},null,2));if(reports.some(r=>r.error||r.checks?.some(c=>!c.pass)))process.exitCode=1;});
